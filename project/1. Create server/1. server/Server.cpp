        ...
        for (i = 0; i < fdSocketInfors.fd_count; i++)
        {
            ...
            if (FD_ISSET(sockTemp, &fdReads))
            {
                if (sockTemp == listenSocket)
                {
                    ...
                    cout << i <<" connected " << sockClient << " ";
                    printf("%d.%d.%d.%d:%d\n",addrClient.sin_addr.S_un.S_un_b.s_b1,addrClient.sin_addr.S_un.S_un_b.s_b2,addrClient.sin_addr.S_un.S_un_b.s_b3,addrClient.sin_addr.S_un.S_un_b.s_b4,addrClient.sin_port);
                ...
                else // client인 경우 쓰레드생성(클라이언트소켓 생성
                {
                    ...
                    cout << "socket = " << i << " recv " << sockTemp << endl;
                    if (recvSize > 0)
                    {
                        for (j = 0; j < fdSocketInfors.fd_count; j++)
                        {
                            send(fdSocketInfors.fd_array[j], recvBuffer, recvSize, 0);
                        }
                    }
                    else
                    {
